...
const i18n = createI18n({
  ...
  messages: {
    es: {
      games: { backToGames: 'Volver', start: 'Comenzar', score: 'Puntuación', electronBuilder: 'Configuración electrónica' },
      learn: { correct: 'Correcto', incorrect: 'Incorrecto', seeResult: 'Ver resultado', nextQuestion: 'Siguiente' },
      element: { electronConfiguration: 'Configuración electrónica' },
    },
    en: {}
  }
})
...
function mountGame() {
  return mount(ElectronConfig, {
    global: { plugins: [i18n, createPinia()] }
  })
}

describe('ElectronConfig.vue', () => {
  ...
  it('shows electron configuration text', async () => {
    const wrapper = mountGame()
    await startGame(wrapper)
    const configText = wrapper.find('.font-mono.font-bold')
    expect(configText.exists()).toBe(true)
  })
  ...
  it('shows total electron count', async () => {
    const wrapper = mountGame()
    await startGame(wrapper)
    expect(wrapper.text()).toMatch(/electrons?/)
  })
